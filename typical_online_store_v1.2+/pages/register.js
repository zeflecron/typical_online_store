...
export default function LoginPage() {
  ...
  // one time method to get the salt
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      // the {...} takes the copy of it, not the actual values
      // this means when it is hashed using salt, it will not be seen on the front end
      // because the inputed values are not modified, but a copy of it is
      let userdata = { ...values };
      userdata.id = globalValues.userCtr;
      userdata.password = bcrypt.hashSync(values.password, globalValues.salt);
      globalValues.userCtr = globalValues.userCtr + 1;
      await globalHandler("PUT", globalValues);
      await userHandler("POST", userdata);
  ...
  return (
    <>
      <Head>
        <title>Typical Online Store - Register</title>
